...
use argot_cli::read_config::toml::read_config_file;

#[test]
fn read_toml_table() {
    let configs: ParserConfig = read_config_file("tests/config_table.toml").unwrap();
...
#[test]
fn read_toml_array() {
    let configs: ParserConfig = read_config_file("tests/config_array.toml").unwrap();
    let entries = HashMap::from([
        ("quiet".to_string(), ConfigEntry::Flag),
        ("q".to_string(), ConfigEntry::Alias { target: "quiet".to_string() }),
        ("verbose".to_string(), ConfigEntry::Count),
